...
так думаю нужно как то попробовать с многопоточностью сделать разделить строку и 
и сверять в потоках, но есть но я пока плохо знаком с потоками 

И вообщем идея такова делю строку пока на 2 части и слева по середину + символов 5 из 2 части искать в разных подходах
пока только такая идея оптимизации  этого кода больше чтобы один поток проверял на палидром левую часть + 5 символов из второго потока 
а второй поток проверял правую часть. Ну вроде так должно ускорить процесс проверки на палидромы

Действия:
1) пойти вообще посмотреть как работать в потоках и думаю начальных знаний мне пока хватить простые потоки написать 
2) реализовать все в потоках 
3) Думать над другими оптимизациями
